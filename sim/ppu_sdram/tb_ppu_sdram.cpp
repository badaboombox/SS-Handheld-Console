// PPU + SDRAM integration TB: identical scene to sim/ppu (golden), but all
// assets live in the modeled SDRAM behind the real controller, and every
// fetch crosses arbiter -> burst cache -> adapter -> sdram_ctrl -> chip.
#include "Vppu_sdram_top.h"
#include "verilated.h"
#include <cmath>
#include <cstdint>
#include <cstdio>
...
// asset generation (identical to sim/ppu/tb_ppu.cpp) 
...
static void putOam(Vppu_sdram_top& top, uint32_t idx, uint32_t x, uint32_t y, uint32_t size,
...
static void putMat(Vppu_sdram_top& top, uint32_t idx, double a, double b, double c, double d,
...
int main(int argc, char** argv)
{
    ...
    if (!top.init_done) { fprintf(stderr, "FAIL: sdram init timeout\n"); return 1; }

    // preload assets through the real controller (nonzero words only;
    // verilated model memory is zero-initialized)
    uint64_t p0 = cycles;
    uint32_t words = 0;
    for (uint32_t a = 0; a < mem.size(); a++)
    {
        if (!mem[a]) continue;
        while (!top.tb_wready) clk();
        top.tb_wen = 1; top.tb_waddr = a; top.tb_wdata = mem[a];
        clk();
        top.tb_wen = 0;
        words++;
    }
    while (!top.tb_wready) clk();
    printf("preloaded %u words in %llu cycles\n", words, (unsigned long long)(cycles - p0));
    ...
    // layer config (identical to golden)
    top.layer_en  = 0xF;
    top.layer_pri = (3 << 6) | (2 << 4) | (1 << 2) | 0;
    uint64_t sx = 0;
    sx |= (uint64_t)(97 & 0x3FF) << 0;
    sx |= (uint64_t)(53 & 0x3FF) << 10;
    sx |= (uint64_t)(21 & 0x3FF) << 20;
    sx |= (uint64_t)(5  & 0x3FF) << 30;
    top.scroll_x_all = sx;
    uint64_t sy = 0;
    ...
    uint64_t f0 = cycles;
    while (!top.frame_done && cycles < 200000000ULL)
    {
        if (top.out_valid)
            fb[top.out_line * W + top.out_x] = top.out_pixel;
        clk();
    }
    if (!top.frame_done) { fprintf(stderr, "FAIL: frame did not complete\n"); return 1; }

    uint64_t fc = cycles - f0;
    printf("frame complete in %llu cycles (%.2f us @100MHz, %.2f us/line) over REAL SDRAM\n",
        (unsigned long long)fc, fc / 100.0, fc / 100.0 / H);
    ...
    FILE* f = fopen("ppu_sdram_frame.ppm", "wb");
    fprintf(f, "P6\n%u %u\n255\n", W, H);
    ...
    fclose(f);
    printf("DONE: ppu_sdram_frame.ppm\n");
    return 0;
}